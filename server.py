...
class HidServer(nn.Module):
    ...
    def train_(self, clients, batch_clients_idx, epoch, interval):
        ...
        for cinde, idx in enumerate(batch_clients_idx):
            client = clients[idx]
            items, items_emb_grad, linear_layers_grad, loss = client.train_(items_emb, linear_layers, epoch, interval, cinde+1)
            ...
            with torch.no_grad():
                ...
                #embeding_layer = embeding_layer.to('cpu')
                #ouput_d = embeding_layer(items.to('cpu'))

            if loss is not None:
                batch_loss.append(loss)

        with torch.no_grad():
            self.items_emb.weight.data.add_(batch_items_emb_grad, alpha=-parameters.lr)
            for i in range(len(linear_layers)):
                self.linear_layers[i].weight.data.add_(batch_linear_layers_grad[i][0], alpha=-parameters.lr)
                self.linear_layers[i].bias.data.add_(batch_linear_layers_grad[i][1], alpha=-parameters.lr)
        return batch_loss
